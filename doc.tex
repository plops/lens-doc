...
\item find intersection $\f$ of ray with interface and refract to
  obtain $\i'$
\item calculate the freespace distance $d$ to the interface
  $d=(\f-\p)/n_e$
\item extend backward along $\i'$ by $\n d$, this results in the
  corrected position $\p'$
...
the refractive index $n_e=1.33$ of the embedding medium (water) as for the light
the medium increases distances.
\end{document}
